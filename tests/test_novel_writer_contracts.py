...
def test_manifest_matches_plugin_contract() -> None:
    ...
    component_names = {getattr(component, "action_name", None) for component in plugin.get_components()}
...
def test_config_field_descriptions_explain_placeholders() -> None:
    ...
    assert "false 时插件不注册 write_novel Action" in fields["enabled"].description
...
def test_build_prompt_uses_core_personality(monkeypatch: Any) -> None:
    ...
    monkeypatch.setattr(
        "plugins.novel_writer.action.get_core_config",
...
def test_get_model_set_uses_actor_task_by_default(monkeypatch: Any) -> None:
    ...
    monkeypatch.setattr(
        "plugins.novel_writer.action.llm_api.get_model_set_by_task",
...
def test_get_model_set_uses_configured_model_name(monkeypatch: Any) -> None:
    ...
    action = _make_action(config)
    monkeypatch.setattr(
        "plugins.novel_writer.action.llm_api.get_model_set_by_name",
